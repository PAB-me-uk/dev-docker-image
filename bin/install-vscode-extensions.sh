...
uninstall_vscode_extensions () {
  ...
  if [[ -f $file ]]; then
    ...
    if [[ $? -eq 0 ]]; then
      ...
      grep -v "^ *#" $file | while IFS="" read -r line ; do
      if [[ ! -z "$line" ]] ; then # Not empty
        result=$(code --uninstall-extension $line)
        fail=$(echo $result | grep -c "Failed Installing Extensions") || true
        echo $fail - $result
        if [[ $fail -ne 0 ]] ; then
          echo Failed
...
uninstall_vscode_extensions /var/dependencies/extensions-to-uninstall.txt
uninstall_vscode_extensions /var/custom-dependencies/extensions-to-uninstall.txt
