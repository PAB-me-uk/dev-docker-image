...
install-terragrunt version:
  #! /bin/bash
  set -eox pipefail
  sudo wget -qO /usr/local/bin/terragrunt https://github.com/gruntwork-io/terragrunt/releases/download/v{{version}}/terragrunt_linux_${ARCH_ALT_NAME}
...
install-terraform version: create-temp-dir && remove-temp-dir
  ...
  set -eox pipefail
  cd {{temp-dir}}
  wget -qO terraform.zip https://releases.hashicorp.com/terraform/{{version}}/terraform_{{version}}_linux_${ARCH_ALT_NAME}.zip
...
install-go version: create-temp-dir && remove-temp-dir
  ...
  set -eox pipefail
  cd {{temp-dir}}
  wget -qO go.tar.gz https://go.dev/dl/go{{version}}.linux-${ARCH_ALT_NAME}.tar.gz
...
install-github-cli version: create-temp-dir && remove-temp-dir
  ...
  set -eox pipefail
  cd {{temp-dir}}
  wget -qO gh.deb https://github.com/cli/cli/releases/download/v{{version}}/gh_{{version}}_linux_${ARCH_ALT_NAME}.deb
  dpkg -i gh.deb
